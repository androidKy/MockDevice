...
public class UdpProvider extends Provider {
    ...
    public UdpProvider(ParcelFileDescriptor descriptor, LocalVpnService service) {
    ...
    protected void forwardPacket(DatagramPacket outPacket, IpPacket parsedPacket) throws LocalVpnService.VpnNetworkException {
        ...
        try {
            ...
            L.i("DNSProvider: Could not send packet to upstream, forwarding packet directly");
        }
    }
     ...
     * @throws Exception If some network error occurred
     */


    @Override
    protected void handleDnsRequest(byte[] packetData) throws LocalVpnService.VpnNetworkException {
        ...
        try {
            ...
            L.e(e.getMessage(),e);
            L.e("handleDnsRequest: DNS server alias query failed for " + destAddr.getHostAddress());
            return;
        }
        ...
        if (parsedUdp.getPayload() == null) {
           ...
           /* DatagramPacket outPacket = new DatagramPacket(new byte[0], 0, 0, destAddr,
            ...
            DatagramPacket outPacket = new DatagramPacket(new byte[0], 0, 0,
        ...
        if (!resolve(parsedPacket, dnsMsg)) {
            DatagramPacket outPacket = new DatagramPacket(dnsRawData, 0, dnsRawData.length, destAddr,
                   parsedUdp.getHeader().getDstPort().valueAsInt());
