...
public abstract class Provider {
    ...
    protected LocalVpnService service;
    ...
    Provider(ParcelFileDescriptor descriptor, LocalVpnService service) {
    ...
    public boolean resolve(IpPacket parsedPacket, DnsMessage dnsMsg) {
        ...
        try {
            ...
            if (response != null && dnsMsg.getQuestion().type == Record.TYPE.A) {
                L.i("Provider: Resolved " + dnsQueryName + "  Local resolver response: " + response);
    ...
    protected void writeToDevice(FileOutputStream outFd) throws LocalVpnService.VpnNetworkException {
        try {
            outFd.write(deviceWrites.poll());
        } catch (IOException e) {
            throw new LocalVpnService.VpnNetworkException("Outgoing VPN output stream closed");
        }
    }

    protected void readPacketFromDevice(FileInputStream inputStream, byte[] packet) throws LocalVpnService.VpnNetworkException {
        // Read the outgoing packet from the input stream.
        int length;
        try {
            length = inputStream.read(packet);
        } catch (IOException e) {
            throw new LocalVpnService.VpnNetworkException("Cannot read from device", e);
        }
        if (length == 0) {
            return;
        }
    ...
    protected abstract void handleDnsRequest(byte[] packetData) throws LocalVpnService.VpnNetworkException;
}
